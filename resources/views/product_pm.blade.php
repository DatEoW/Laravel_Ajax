            ...
            <div style="display: flex;gap:80px;flex-wrap: wrap;" class="form_search">
                ...
                <div>
                    ...
                    <button id="resetBtn" type="reset" class="btn btn-info mb-3"
                        style="margin-bottom:40px !important;margin-top:40px;margin-right: 10px;"><i class="fa-solid fa-circle-xmark"
...
<script>
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    for (let i = 0; i < items.length; i++) {
                        let sales_status=response[0].data[i]?.is_sales;
                        if (response[0].data[i].is_active === 0) {
                            response[0].data[i].is_active = false
                        }
                        if(Number(sales_status)===0){
                            sales_status='<strong style="color:red">Ngừng bán</strong>';
                        }else if(Number(sales_status)==1){
                            sales_status='Đang bán';
                        }else if(Number(sales_status)==2){
                            sales_status='Hết hàng';
                        }
                        let id=response[0].data[i]?.id;

                        let route_edit=`{{ route('product.edit',['product' => ':productId']) }}`.replace(':productId',id);
                        $('#tbody-table').append(`
                            <tr>
        ...
        $('#deleteBtn').click(function(event) {
            ...
            $.ajax({
                url: `{{ route('product.destroy',['product' => ':productId']) }}`.replace(':productId',id),
                method: 'delete',
                success: function(response) {
                    $('.ajax-modal').modal('hide');
                    renderTable (page,perPage,name,priceMin,priceMax,is_sales);
