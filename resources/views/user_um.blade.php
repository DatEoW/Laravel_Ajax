            ...
            <div style="display: flex;gap:80px;flex-wrap: wrap;" class="form_search">
                <div>
                    <a href="javascript:void(0)"
                        ...
                        @if (Auth::user()->group_role != 0) class="btn btn-info mb-3 disabled" @else class="btn btn-info mb-3" @endif>
...
<script>
    ...
    $(document).ready(function() {
                ...
                success: function(response) {
                    ...
                    for (let i = 0; i < items.length; i++) {
                        ...
                        $('#tbody-table').append(`
                            ...
                            <td><a href="javascript:void(0)" data-id="${response[0].data[i]?.id}" data-role="${response[0].data[i]?.group_role}" class="btn btn-primary editButton ${roleUser?'disabled':''}" data-bs-toggle="modal"
                        ...
                        <a id="lock${response[0].data[i]?.id}"  href="javascript:void(0)" data-id="${response[0].data[i]?.id}" data-role="${response[0].data[i]?.group_role}" data-active="${response[0].data[i]?.is_active?1:0}" class="btn btn-info toLock ${roleUser?'disabled':''}" data-bs-toggle="modal"  data-bs-target="#lockModal"
        ...
        $('#deleteBtn').click(function(event) {
            ...
            $.ajax({
                url: `{{ route('deleteUser') }}/?id=${id}&phuongThuc=${phuongThuc}`,
                method: 'POST',
                success: function(response) {
                    $('.ajax-modal').modal('hide');
                    renderTable(page, perPage, name, email, group_role, is_active);
        ...
        $('#lockBtn').click(function(event) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    renderTable(page, perPage, name, email, group_role, is_active);
                    if (response) {
                        if(response.responseJSON.phuongThuc==='lock'){
                            Swal.fire({
